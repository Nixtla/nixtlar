...
TimeGPT](https://www.nixtla.io/docs), a generative pre-trained
forecasting model for time series data. `TimeGPT` is the first
foundation model capable of producing accurate forecasts for new time
series not seen during training, using only its historical values as
inputs. `TimeGPT` can also be used for other time series related tasks,
such as anomaly detection and cross-validation. Here we explain how to
get started with `TimeGPT` in R and give a quick overview of the main
features of `nixtlar`.
...
[here](https://www.nixtla.io/dashboard).
